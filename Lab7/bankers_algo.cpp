...
bool is_safe(int taken_p,vector<bool>&completed)
{
    ...
    for(int p=1;p<=num_process;p++){if(!completed[p] && p!=taken_p){all_complete = false; break;}}
    if(all_complete){return true;}
    
    for(int p=1;p<=num_process;p++)
    {
        if(completed[p] || p==taken_p){continue;}
        
        bool ans = true;
        
        for(int r=1;r<=num_resource;r++)
        {
            if(need[p][r]>available[r]){ans = false; break;}
        }
...
bool is_allowed(int p,vector<bool>&completed)
{
    for(int r=1;r<=num_resource;r++)
    {
        if(need[p][r]>available[r]){return false;}
    }
    
    for(int r=1;r<=num_resource;r++){available[r]+=allocated[p][r];}
...
{
    bool all_complete = true;
    for(int p=1;p<=num_process;p++){if(!completed[p]){all_complete = false; break;}}
    if(all_complete){return true;}
...
bool is_safe_req(int req_proc)
{
    for(int p=1;p<=num_process;p++)
    {
        ...
        for(int r=1;r<=num_resource;r++)
        {
            if(need[p][r]>available[r]){allowed = false; break;}
        }
...
bool is_allowed_req(int req_proc,vector<int>&request)
{
    for(int r=1;r<=num_resource;r++)
    {
        if(request[r]>available[r]){cout<<"Request for resource "<<r<<" is greater than available!\n"; cout<<"\n"; return false;}
        if(request[r]>need[req_proc][r]){cout<<"Unsafe, if immediately allocated "<<r<<" will not have enough resources to continue.\n"; cout<<"\n"; return false;}
    }
    ...
    cout<<"Resources available : "; for(int r=1;r<=num_resource;r++){cout<<available[r]<<" ";} cout<<"\n";
    cout<<"\n"; cout<<"\n";
    ...
    if(!is_safe_req(req_proc)){
        ...
        cout<<"System becomes unsafe after allocation\n"; cout<<"\n";
        return false;
    }
    ...
    for(int r=1;r<=num_resource;r++)
    {
        if(need[req_proc][r]>0){all_satisfied = false; break;}
    }
...
int main()
{
    ...
    for(int p=1;p<=num_process;p++)
    {
        for(int r=1;r<=num_resource;r++){need[p][r] = max_allowed[p][r] - allocated[p][r];}
    }
    
    available = vector<int>(num_resource + 1);
    cout<<"enter availble resources";
    for(int r=1;r<=num_resource;r++){cin>>available[r];}
    ...
    if(solve(sequence,completed)){
        ...
        cout<<"Safe sequence is :"; for(auto x : sequence){cout<<x<<" ";}
    }
    else{
